...
* Option 1 is a _high-level overview_ of why you might want to develop a package
* Option 2 is a more _code-oriented walkthrough_ of what you can do with a package

All the slides are online: [bit.ly/tu-pkgs](https://bit.ly/tu-pkgs)
...
* Generally a _collection of code_ for some specific purpose
* In R, usually distributed _open source_ & free of charge
...
pal_mtx <- stylehaven::palx("#f137e8", n_hues = 10, n_shades = 5, plot = FALSE)
...
The _dependency_ of my _dependency_ is also my _dependency_
...
exports |>
    ...
    scale_fill_manual(values = c("gray60", unname(pal[c(2, 3)]))) +
    theme(
        legend.position = "inside",
...
* _Standardize_ pieces of information (geographies, definitions)
* _Reduce repetition_, access to utilities
* Collect _resources_ for teaching & onboarding
...
Instead of scattered spreadsheets, text files, shapefiles, info stored in one person's brain, _codify information_ into datasets, objects, rules, etc. For example:
...
* _Batch_ download Census data (multiple tables, geographies, years)
* Do _aggregations_ or other calculations
* Handle complicated _parameters_ or other pain points
* Move to _parallel processing_
...
(Feel free to repurpose the package in your own teaching: [umbc-viz.github.io/justviz](https://umbc-viz.github.io/justviz){preview-link="true"})
...
* Download a 2022 5-year ACS table on housing tenure for Connecticut
    ...
    - Then download the same table for every tract in every county
* Aggregate values for the towns in Greater New Haven
    - Then aggregate the towns in Greater Hartford
    - Then aggregate the towns in Fairfield County
* Aggregate values for the tracts that make up each neighborhood in New Haven, with weights
    - Then aggregate the neighborhoods in Hartford, with another table of weights
    - Then aggregate the neighborhoods in Bridgeport, with another nother table of weights
:::
...
A common guideline is if you do the same thing 3 times, _you should write a function_. ^[See Wickham, _R for Data Science_] In this case, I could write a function that takes several inputs:
...
* Clear _documentation_ of every function, dataset, and class
* Handling _edge cases_ & errors, incl. preempting user error
* Unit _testing_
* _Bug fixes_ & routine maintenance
* Keeping up with _dependency_ updates & changes
...
* For managers / directors: please _invest in your employees' time_ to develop own tools
...
{{< iconify fluent branch-fork-24-regular >}} Adapt my {`cwi`} package for your area: [{{< iconify fe github-alt >}}/CT-Data-Haven/cwi](https://github.com/CT-Data-Haven/cwi)
